...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        window.location.href = '/login/loginpage.html'; // Redirect if not logged in
    }
});
...
const updateNavbar = (user) => {
  ...
  const userAvatarElem = document.getElementById("loginbutton");
  ...
  if (user) {
    ...
    // Update username (displayName or email)
    userNameElem.textContent = user.displayName || user.email.split("@")[0];

    // Update avatar (use default if none exists)
    userAvatarElem.src = user.photoURL || "/images/default-avatar.png";
